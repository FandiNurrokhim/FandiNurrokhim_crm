...
const CreateModal = ({ isOpen, onClose, setRefetch }) => {
    ...
    // Ambil data dropdown saat modal dibuka
    useEffect(() => {
        if (isOpen) {
    ...
    useEffect(() => {
        setData("products", dealProducts.map(item => ({
    ...
    return (
        <Modal show={isOpen} onClose={onClose} title="Tambah Deal"
            type="add"
            maxWidth="3xl"
            onCancel={() => {
                onClose();
